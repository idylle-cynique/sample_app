...
class ForgotPasswordFormTest < PasswordResets
  ...
  # パスワードが空の場合パスワードリセットが完了できないことを確認
...
class PasswordFormTest < PasswordResetForm
  # 適切なメールアドレスでパスワードリセット用のリクエストを行ったとき
  ...
  # 正しいメールアドレスと不適切なトークンでパスワードリセット用のリクエストを送ったとき
...
class PasswordUpdateTest < PasswordResetForm
  ...
  # 適切なパスワード値を入力することでパスワードの再設定が出来る
